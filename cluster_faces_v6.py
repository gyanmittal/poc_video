...
# ---- v6 override: add audio mux ----
_orig_annotate = annotate_segment_opencv
def annotate_segment_opencv(video_src: str, start_sec: float, end_sec: float,
                            cluster_frame_boxes: Dict[int, Tuple[int, int, int, int]],
                            cfg: FaceClusterConfig, out_path: str) -> bool:
    video_only_tmp = out_path + ".vtmp.mp4"
    ok = _orig_annotate(video_src, start_sec, end_sec, cluster_frame_boxes, cfg, video_only_tmp)
    if not ok:
        try:
